...
def build_queue_embed(
    current_song, queue: list, page: int, items_per_page: int = QUEUE_ITEMS_PER_PAGE
) -> discord.Embed:
...
class MusicCog(commands.Cog):
        ...
        if voice_client is None and should_connect:
            try:
                ...
                try:
                    ...
                    await ui.ErrMsg.msg(
                        interaction, "Timed out while trying to connect to voice channel. Please try again."
                    )
                    return None
    ...
    async def play_next_prefix(self, ctx: commands.Context, *, query: str):
        ...
        player.channel = ctx.channel
        await player.enqueue(songs[0], position="front")
        await ctx.send(f"Queued **{songs[0].title}** by {songs[0].artist}")
        await player.play_audio_queue(voice_client)
    ...
    async def play_query_autocomplete(
        ...
        if "querytype" not in interaction.namespace or interaction.namespace["querytype"] == "track":
            if current == "":
                ...
                songs = (
                    await subsonic.search(
                        current, artist_count=0, album_count=0, song_count=env.BOT_SEARCH_SUGGESTION_COUNT
                    )
                ).songs
            choices = [
            ...
            if current == "":
                ...
                albums = (
                    await subsonic.search(
                        current, artist_count=0, album_count=env.BOT_SEARCH_SUGGESTION_COUNT, song_count=0
                    )
                ).albums
            choices = [
    ...
    @app_commands.describe(
        querytype="Whether what you're searching is a track, album or playlist", query="Enter a search query"
    )
    ...
    async def play(self, interaction: Interaction, querytype: str = None, query: str = None) -> None:
        ...
        if query is None:
            ...
            if (
                player.queue == []
                and data.guild_properties(interaction.guild_id).autoplay_mode == data.AutoplayMode.NONE
            ):
        ...
        if querytype == "track" or querytype is None:
            ...
            if songs == "Error":
                await ui.ErrMsg.msg(
                    interaction,
                    "An api error has occurred and has been logged to console. Please contact an administrator.",
                )
                return
            ...
            await player.extend_queue(album.songs)
            ...
            await player.extend_queue(playlist.songs)
    ...
    async def play_error(self, ctx, error):
        if isinstance(error, subsonic.APIError):
            ...
            await ui.ErrMsg.msg(
                ctx, "An API error has occurred and has been logged to console. Please contact an administrator."
            )
        else:
            logging.error(f"An error occurred while playing a track: {error}")
            await ui.ErrMsg.msg(
                ctx,
                f"An unknown error has occurred and has been logged to console. Please contact an administrator. {error}",
            )
    ...
    @app_commands.describe(
        action="Queue action to perform", index="Queue index to edit", target="Queue position to move the track to"
    )
    ...
    async def show_queue(
        self, interaction: Interaction, action: str = "list", index: int = None, target: int = None
    ) -> None:
        """Show or edit the current queue"""

        player = data.guild_data(interaction.guild_id).player
        queue = player.queue
        action_value = choice_value(action) or "list"

        if action_value not in ("list", "remove", "move"):
            await ui.ErrMsg.msg(interaction, "Unknown queue action.")
            return
        ...
        if action_value != "list":
            ...
            if action_value == "remove":
                removed_song = await player.remove_queued_song(index - 1)
            ...
            moved_song = await player.move_queued_song(index - 1, target - 1)
            await ui.SysMsg.msg(
                interaction, "Moved queue track", f"{format_queue_song(moved_song)}\n{index} -> {target}"
            )
            return
    ...
    async def clear_queue(self, interaction: Interaction) -> None:
        """Clear the queue"""
        await data.guild_data(interaction.guild_id).player.clear_queue()
    ...
    async def autoplay(self, interaction: Interaction, mode: app_commands.Choice[str]) -> None:
        ...
        mode_value = choice_value(mode)
        logger.debug(f"Autoplay mode: {mode_value}")
        ...
        match mode_value:
            ...
            case _:
                await ui.ErrMsg.msg(interaction, "Unknown autoplay mode.")
                return
        ...
        if mode_value == "none":
            ...
            await ui.SysMsg.msg(
                interaction,
                f"Autoplay enabled by {interaction.user.display_name}",
                f"Autoplay mode: **{choice_name(mode)}**",
            )
    ...
    async def audiobalance(self, interaction: Interaction, mode: app_commands.Choice[str]) -> None:
        ...
        match mode_value:
            ...
            case _:
                await ui.ErrMsg.msg(interaction, "Unknown audio balance mode.")
                return
        data.save_guild_properties_to_disk()

        await ui.SysMsg.msg(
            interaction,
            f"Audio balancing set to {choice_name(mode)} by {interaction.user.display_name}",
    ...
    async def shuffle(self, interaction: Interaction):
        ...
        await data.guild_data(interaction.guild_id).player.shuffle_queue(shuffledqueue)
    ...
    async def disco_artist_autocomplete(
        ...
        artists = (
            await subsonic.search(current, artist_count=env.BOT_SEARCH_SUGGESTION_COUNT, album_count=0, song_count=0)
        ).artists
        return [app_commands.Choice(name=artist.name, value=artist.name) for artist in artists]
    ...
    async def disco(self, interaction: Interaction, artist: str):
        ...
        await player.extend_queue([song for album in albums for song in album.songs])
    ...
    async def disco_error(self, ctx, error):
        if isinstance(error, subsonic.APIError):
            logging.error(
                f"An API error has occurred while playing an artist's discography, code {error.code}: {error.message}"
            )
            await ui.ErrMsg.msg(
                ctx, "An API error has occurred and has been logged to console. Please contact an administrator."
            )
    ...
    async def list_playlist(self, interaction, query: str = None, page: int = 1):
        if query is None:
            await ui.ErrMsg.msg(interaction, "Please provide the name of a playlist.")
            return
        ...
        for _i, pag_item in enumerate(paginated_tracks.pages[page - 1]):
            song = pag_item["data"]
    ...
    async def on_voice_state_update(
        ...
        if len(voice_client.channel.members) == 1:
            ...
            if len(voice_client.channel.members) == 1:
                ...
                logger.info(
                    "The bot has disconnected and cleared the queue as there are no users in the voice channel."
                )
            else:
